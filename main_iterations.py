...
from accuracy import create_accuracy_dictionary, write_csv, write_param
...
if __name__ == "__main__":
    ...
    parser.add_argument("-modn", "--model_name", type=str, help="name of the anmodel we are using", default="QUEST_CNN")
    ...
    parser.add_argument("-e_flag", "--embedding_flag", type=int,
                        help="  1 use google embedding, 2 use mimic dataset, 3 random start", default=2)
    ...
    parser.add_argument("-dre", "--dropout_embedding", type=float,
                        help="dropout embedding for cnn_text", default=0.5)
    parser.add_argument("-inter", "--intermidiate", type=int,
                        help="size of the intermidiate layer for cnn_text", default=256)
    parser.add_argument("-fm", "--feature_maps", type=int,
                        help="size of feature map for each filter", default=64)
    ...
    parser.add_argument("-dyn", "--dynamic_pool", type=int,
                        help="size of dynamic pooling map", default=2)
    ...
    seeds = [6809, 36275, 5317, 82958, 25368]
    # seeds = [6809, 36275 ]
    ...
    for seed in seeds:
        ...
        if args.model_name == "QUEST_CNN":
            ...
            model = BI_LSTM(args, args.class_number, args.data_name, embedding.embedding_matrix,
                            ...
                            upos=args.upos, umed=args.umed, uad=args.uad, qk=args.question_knowledge)
        ...
        accuracy_dictionary, f1_model = network_iteration.model_eval(accuracy_dictionary, test=True, time=run_end,
                                                                     voc=data.number_to_word)
        print(f1_model)
        accuracy_dictionary_val, f1_model = network_iteration.model_eval(accuracy_dictionary_val, test=False,
                                                                         time=run_end, voc=data.number_to_word)
        parameters = running_network.get_parameters()

    name1 = "dataset_output/results/results_" + args.data_final_name + ".csv"
    name2 = "dataset_output/results/results_" + args.data_final_name + "_val.csv"

    write_csv(name1, accuracy_dictionary)
    write_csv(name2, accuracy_dictionary_val)
    name1 = "dataset_output/results/results_param_" + args.model_name + ".csv"
